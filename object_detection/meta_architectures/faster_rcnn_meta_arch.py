...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def __init__(self,
        ...
        Args:
          ...
          rpn_type' is the type of rpn which is 'cascade_rpn','orign_rpn'
            ...
            generated by rpn
          filter_fn: if not None, it will filter the proposals generated by RPN after
            the first_stage_nms.
          replace_rpn_arg: a dictionary. only if the rpn_type=='without_rpn' and not None, it's useful in
            order to replace the proposals generated by rpn with the gt which maybe adjusted.
            'type': a string which is 'gt' or 'rpn', default to 'gt'
        ...
        self._replace_rpn_arg = replace_rpn_arg
        self._filter_fn = filter_fn
        self._filter_boxes_list = None
        self._rpn_boxes_list = None
        self._rpn_objectness_predictions_with_background = None
    ...
    @property
    def first_stage_box_predictor_cascade_scope(self):
        return 'FirstStageBoxCascadePredictor'
    ...
    def predict(self, preprocessed_inputs, true_image_shapes):
        ...
        if not self._is_training and self._rpn_type == 'without_rpn':
            if self._replace_rpn_arg:
                if self._replace_rpn_arg.get('type', 'gt') == 'gt':
                    proposal_bboxes, proposal_scores, vaildation = self._get_gt_data(
                        scale=self._replace_rpn_arg.get('scale', 1.0))
                else:
                    proposal_bboxes, proposal_scores, vaildation = self._get_rpn_data(
                        scale=self._replace_rpn_arg.get('scale', 1.0))
            else:
                proposal_bboxes, proposal_scores, vaildation = self._get_rpn_data()
            ...
            if self._number_of_stages >= 2:
                ...
                prediction_dict.update(self._predict_second_stage_rcnn(
                    rpn_features_to_crop, image_shape, proposal_bboxes, proposal_scores, vaildation))
        else:
                ...
                if self.clip_anchors_to_image:
                    ...
                    if self._rpn_type == 'cascade_rpn':
                        (rpn_box_encodings, rpn_objectness_predictions_with_background, rpn_box_encodings_cascade,
                         ...
                         anchors_boxlist) = self._remove_invalid_anchors_and_predictions_and_cascade(
                            rpn_box_encodings, rpn_objectness_predictions_with_background, rpn_box_encodings_cascade,
                            rpn_objectness_predictions_with_background_cascade,
            ...
            if self._number_of_stages >= 2:
                ...
                prediction_dict.update(self._predict_second_stage(
                    ...
                    rpn_objectness_predictions_with_background_cascade=rpn_box_predictor_features_cascade))
    ...
    def _predict_second_stage(self, rpn_box_encodings,
                              ...
                              anchors,
                              image_shape,
                              true_image_shapes, rpn_box_encodings_cascade=None,
                              rpn_objectness_predictions_with_background_cascade=None
                              ):
        ...
        Returns:
          prediction_dict: a dictionary holding "raw" prediction tensors:
            ...
            7) proposal_boxes_scores: A float tensor with shape
        ...
        if self._rpn_type == 'cascade_rpn':  # ???using which score
    ...
    def _predict_second_stage_rcnn(self,
        ...
        Args:
          rpn_features_to_crop: A 4-D float32 or bfloat16 tensor with shape
            ...
            using the proposal boxes predicted by the RPN.
          image_shape: A 1D int32 tensors of size [4] containing the image shape.
          proposal_boxes_normalized: A float32 tensor of shape
              [batch_size, self.max_num_proposals, 4] representing decoded proposal
              bounding boxes in normalized coordinates. Can be used to override the
              boxes proposed by the RPN, thus enabling one to extract features and
              get box classification and prediction for externally selected areas
              of the image.
          proposal_boxes_scores: A float32 tensor with shape
              [batch_size, max_num_proposals] representing the (potentially zero
              padded) proposal objectness scores for all images in the batch.
    ...
    def _extract_rpn_feature_maps(self, preprocessed_inputs):
        ...
        Returns:
          ...
          rpn_box_predictor_features_cascade: only useful when rpn_type=='cascade_rpn'
            A 4-D float32 tensor with shape [batch, height, width, depth] to be used
            for predicting proposal boxes and corresponding objectness scores in cascade.
        """
    ...
    def _predict_rpn_proposals(self, rpn_box_predictor_features, scope=None):
    ...
    def _remove_invalid_anchors_and_predictions_and_cascade(
            self,
            box_encodings,
            objectness_predictions_with_background,
            box_encodings_cascade,
            objectness_predictions_with_background_cascade,
            anchors_boxlist,
            clip_window):
        """Removes anchors that (partially) fall outside an image.
        ...
        return (_batch_gather_kept_indices(box_encodings),
                _batch_gather_kept_indices(objectness_predictions_with_background),
                _batch_gather_kept_indices(box_encodings_cascade),
                _batch_gather_kept_indices(objectness_predictions_with_background_cascade),
                pruned_anchors_boxlist)
        ...
        if not self._is_training and self._filter_fn:
            groundtruth_boxlists = tf.convert_to_tensor([
                box_list_ops.to_absolute_coordinates(
                    ...
                    true_image_shapes[i, 1]).get()
                for i, boxes in enumerate(
                    self._get_filter_box())])
            filter_boxes = groundtruth_boxlists
            proposal_boxes, proposal_scores, num_proposals, max_proposals_filter = self._filter_fn(proposal_boxes,
                                                                                                   filter_boxes,
                                                                                                   proposal_scores)
            self._first_stage_max_proposals = max_proposals_filter
    ...
    def _loss_rpn(self, rpn_box_encodings,
                  ...
                  groundtruth_boxlists, groundtruth_classes_with_background_list,
                  groundtruth_weights_list, rpn_box_encodings_cascade=None,
                  rpn_objectness_predictions_with_background_cascade=None):
        ...
        with tf.name_scope('RPNLoss'):
            ...
            if self._rpn_type == 'cascade_rpn':
                ...
                objectness_cascade_losses = self._first_stage_objectness_loss(
                    ...
                    weights=tf.expand_dims(batch_sampled_indices, axis=-1),
                    losses_mask=losses_mask)
                localization_cascade_losses = tf.reduce_mean(
                    tf.reduce_sum(localization_cascade_losses, axis=1) / normalizer)
                objectness_cascade_losses = tf.reduce_mean(
                    tf.reduce_sum(objectness_cascade_losses, axis=1) / normalizer)
                localization_cascade_losses = tf.multiply(self._first_stage_loc_loss_weight,
                                                          localization_cascade_losses,
                                                          name='localization_cascade_loss')
                objectness_cascade_losses = tf.multiply(self._first_stage_obj_loss_weight,
                                                        objectness_cascade_losses, name='objectness_cascade_loss')
                loss_dict.update({localization_cascade_losses.op.name: localization_cascade_losses,
                                  objectness_cascade_losses.op.name: objectness_cascade_losses})
        return loss_dict
    ...
    def _scale_bbox(self, boxes, scale=1.0):
        """scale the height and width of the boxes with scale.
            Args:
              boxes: a float tensor of [nums, 4] containing (normalized) coordinates
                 of the boxes. the format is ymin,xmin,ymax,xmin
              scale: a float which scale the box
            Returns:
               a float tensor of [nums, 4] containing (normalized) coordinates
                of the boxes. the format is ymin,xmin,ymax,xmin
            """
        ...
        ymin = tf.maximum(0.0, ymin + height * (1 - scale) / 2)
        xmin = tf.maximum(0.0, xmin + width * (1 - scale) / 2)
        ymax = tf.minimum(1.0, ymax + height * (scale - 1) / 2)
        xmax = tf.minimum(1.0, xmax + width * (scale - 1) / 2)

        return tf.transpose(tf.stack([ymin, xmin, ymax, xmax]))

    def _per_batch_padding(self, args):
        """Returns proposal_boxes_result, proposal_score_result and validation.

        Args:
            args[0]: boxes. A float tensor with shape [num_proposals, ...].
            args[1]: max_number. A int tensor with shape [,] representing
                   the kept boxes's number with padding zeros

            Returns:
                result: A float tensor with shape
                    [max_number, ...] representing the (potentially zero
                    padded) boxes for all images in the batch.
                vaildation: A int tensor with shape
                    [,] representing the validative proposal boxes for the batch.
            """
        boxes = args[0]
        max_number = args[1]
        vaildation = shape_utils.combined_static_and_dynamic_shape(boxes)[0]
        boxes_result = shape_utils.pad_or_clip_tensor(
            boxes, max_number)
        vaildation = tf.min(vaildation, max_number)
        return [boxes_result, vaildation]

    def _padding_list_to_same(self, boxes_list, max_number):
        """convert the list of tensor to tensor with shape [list_size,...]
           Returns boxes_result, vaildation.

        Args:
            boxes_list: A list of tensor with shape [num_proposals, ...].
            max_number: A int tensor with shape [,] representing
                   the kept boxes's number with padding zeros

        Returns:
            boxes_result: A float tensor with shape [batch_size,max_number, ...]
                representing the (potentially zero padded) boxes for all images in the batch.
            vaildation: A int tensor with shape [batch_size, ] representing
                the validative proposal boxes for the batch.
        """
        boxes_result = []
        vaildations = []
        for box in boxes_list:
            vaildation = shape_utils.combined_static_and_dynamic_shape(box)[0]
            boxes_result.append(shape_utils.pad_or_clip_tensor(box, max_number))
            vaildations.append(tf.minimum(vaildation, max_number))
        return [tf.convert_to_tensor(boxes_result), tf.convert_to_tensor(vaildations)]

    def _get_gt_data(self, scale=None, agnostic=True):
        """Helper function for preparing groundtruth data for replacing the proposal
           generated by rpn

        Args:
          scale: a float which scale the boxes. default is None that represent the boxes do not need change.
          agnostic: boolean. if False, the boxes return will has shape[batch_size,num_boxes, num_class],
              otherwise the shape is [batch_size, num_boxes, 1]

        Returns:
          groundtruth_boxs: A float tensor of [batch_size, num, 4] containing (normalized) coordinates
            of the groundtruth boxes. maybe padding by zero.
          groundtruth_classes_with_backgrounds: A float tensor of shape [batch_size, num_boxes, num_classes]
            containing the class targets with the 0th index assumed to map to the first class which is not
            background. maybe padding by zero
          vaildations: A int tensor of shape [batch_size,]
        """

        groundtruth_boxs = self.groundtruth_lists(fields.BoxListFields.boxes)
        groundtruth_boxs, vaildations = self._padding_list_to_same(groundtruth_boxs, self.max_num_proposals)
        groundtruth_boxs_shape = shape_utils.combined_static_and_dynamic_shape(groundtruth_boxs)
        if scale and scale > 0:
            groundtruth_boxs = self._scale_bbox(tf.reshape(groundtruth_boxs, [-1, 4]), scale=scale)
            groundtruth_boxs = tf.reshape(groundtruth_boxs, groundtruth_boxs_shape)
        if agnostic:
            groundtruth_classes_with_backgrounds = tf.ones([groundtruth_boxs_shape[0], groundtruth_boxs_shape[1], 1])
        else:
            groundtruth_classes_with_backgrounds = tf.convert_to_tensor(
                self.groundtruth_lists(fields.BoxListFields.classes))
        return (groundtruth_boxs, groundtruth_classes_with_backgrounds, vaildations)

    def provide_rpn_box_list(self, rpn_boxes_list):
        self._rpn_boxes_list = rpn_boxes_list

    def _get_rpn_data(self, scale=None):
        rpn_boxes, vaildations = self._padding_list_to_same(self._rpn_boxes_list, self.max_num_proposals)
        rpn_boxes_shape = shape_utils.combined_static_and_dynamic_shape(rpn_boxes)
        if scale and scale > 0:
            rpn_boxes = self._scale_bbox(tf.reshape(rpn_boxes, [-1, 4]), scale=scale)
            rpn_boxes = tf.reshape(rpn_boxes, rpn_boxes_shape)
        rpn_objectness_predictions_with_background = tf.ones([rpn_boxes_shape[0], rpn_boxes_shape[1], 1])
        return (rpn_boxes, rpn_objectness_predictions_with_background, vaildations)

    def provide_filter_box_list(self, filter_boxes_list):
        """
            filter_boxes_list:  A list with batch_size of float tensors with shape
                [num_filter, 4] representing the coordinates in range[0, 1].
        """
        self._filter_boxes_list = filter_boxes_list

    def _get_filter_box(self):
        return self._filter_boxes_list
